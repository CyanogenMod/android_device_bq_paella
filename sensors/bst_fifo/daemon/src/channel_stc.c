...
extern int hw_step_counter_enable(BS_U8 enable);
extern int hw_step_counter_mode(BS_U8 mode);
...
int get_data_stc(void *pbuf, int n)
{
	...
	sensor_data_t *pdata = &(pmsg->data);
	UNUSED_PARAM(n);
	return algo_get_proc_data_stc(pdata);
...
int channel_stc_enable(struct channel *ch, int en)
{
	...
	{
		hw_step_counter_enable(en);
		hw_step_counter_mode(0);
	}
	return 0;
 }
