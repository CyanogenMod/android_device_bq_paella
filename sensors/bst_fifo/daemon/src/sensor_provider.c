...
void sp_recalc_interval_re(struct sensor_provider *sp)
{
	...
	PINFO("new re->interval for sp is: %d", re->interval);
}
...
/*
 * @brief This function is to query whether sensor provider is enabled
 *
 * @param none
 *
 * @return TRUE or FALSE
 */
int32_t sensor_provider_is_enabled(void)
{
	if (g_sp_algo_fusion.sp.ref > 0)
	{
		return TRUE;
	}
...
void* re_proc(void* pparam)
{
	...
	while (1) {
		...
		num = 0;
		while (NULL != cur) {
				/* channel means virtual sensor, browse all enabled virtual sensors, and check report data */
			ch = CONTAINER_OF(cur, struct channel, client);
			...
			if ((CHANNEL_STATE_NORMAL == ch->state)
				...
				if (ch->cfg.no_delay ||
					...
					if (ret > 0) {
						...
						data[num].ts = (int64_t)time_start * 1000;
						num ++;
					}
